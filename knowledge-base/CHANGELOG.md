...
## 2026-06-16 — Fix(M10): enforce form-level rules on submit (gps/photo/check-in)

**By:** Tech lead (MotivesVN IT), AI-assisted

**Status:** ✅ Backend 0 errors, **299 unit tests** (+ form-rules suite). Migration `M10_FormSubmissionGps` applied to dev DB.

- **Bug (confirmed via DB):** the Form Builder saves `schema.rules` (`require_check_in` / `gps_required` / `photo_required`) — verified in form code `1` v2 — but `SubmitFormCommandHandler` only ran `FindMissingRequired` and **ignored `rules`** entirely, so they were never enforced.
...
- **Input validation:** added FluentValidation validators for the newer write commands that only validated inline — `UploadDocumentCommand` (name required ≤255, folder required) and `UpdateNewsCommand` (bilingual titles required ≤255) — so they return the standard structured 400 envelope via the validation behavior. (292 tests.)
- ⚠️ Restart the dev API to pick up the fallback policy. Further S17 candidates: rate-limit coverage, more report indexes, error-envelope consistency.
...
**Status:** ✅ Web/mobile i18n key parity verified; release notes drafted. Mobile code-only — **Mac runs `flutter gen-l10n` + `flutter analyze`**.
...
- **Ops scripts + runbook** for the release checklist: [`scripts/smoke-prod.ps1`](../scripts/smoke-prod.ps1) — read-only production smoke test (health → admin login → one GET per shipped Phase 1A module + RBAC-401 negative; prod-safe, no seed/mutate; routes verified against controllers); [`scripts/backup-restore.sh`](../scripts/backup-restore.sh) — `pg_dump -Fc` + tar the bind-mounted MinIO/CompreFace data, then **restore-verify** into a throwaway `--tmpfs` Postgres container with sanity row counts (never touches prod) — operationalises the "dump before recreating the Postgres container" rule. Documented in [OPS-RUNBOOK-Phase-1A.md](./OPS-RUNBOOK-Phase-1A.md). Both linted (`bash -n` + PS tokenizer).
