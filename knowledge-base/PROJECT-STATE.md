...
**Current phase:** Phase 1A — **Sprint 02 in progress.** ✅ M02 device-approval **BE** done (GET `/devices/me`, GET `/devices/pending`, POST `/devices/{id}/approve|reject` — BR-106; approve replaces old device + revokes its refresh tokens; Admin-scoped, Leader scoping deferred to M03). Backend 106 unit tests + AC-3 integration test. **Next:** web Device Requests UI, then M03 (stores/areas/assignments). _(Sprint 01 (M01) closed; Day 10 manual UAT/demo skipped.)_
