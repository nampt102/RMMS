...
**Last updated:** 2026-06-06 — **M07 Work Schedule built ahead of M05 (sprint reorder).** To supply shift times that AC-5/AC-6 attendance checks require, M07 was pulled before M05: BE complete (WorkSchedule aggregate + WorkScheduleShift owned collection, migration `M07_WorkSchedule`, create/submit/edit-with-versioning/withdraw/approve-reject use-cases, Leader-scoped approval, **16 new unit tests → full suite 165 green**); web Admin schedule overview (`/schedules`: pick user + range, view shifts/status, approve/reject) — type-check/lint/build green; mobile Flutter schedule feature (models/api/repo + MyScheduleScreen + RegisterScheduleScreen day/week/month) **code-only, awaiting Mac verify**. Earlier same day: Sprint 02's two deferred items cleared (store map ADR-010 + mobile FCM client-side).
